...
//create personne 
	...
	//$obj->relationPersonne();
		...
		foreach ($music_filter as $key => $value) {
			# code...
			echo $obj->RelationMusique($personne_id,$key);
		}
				...
				<form>
					...
					<?php
						...
						foreach($personne as $key){
								...
								echo '<p><label><input type="checkbox" name="'.$key->Prenom.'">'.$key->Prenom.' '.$key->Nom.'<input type="text" name="'.$key->Prenom.'"></label></p>';
